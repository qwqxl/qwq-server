...
### 打包win to linux
```bash
go build -o qwq-server -ldflags "-s -w" -trimpath -v -tags=jsoniter -race -a -installsuffix cgo GOOS=linux GOARCH=amd64 ./cmd/server/main.go
```
...
docker run -p 5000:5000 -p 80:80 -p 443:443 -name qwq-server \
  -v /qwq/qwq-server/resources:/app/resources \
  -v /qwq/qwq-server/configs:/app/configs \
  -v /qwq/qwq-server/web:/app/web \
  ./qwq-server

docker build -t qwq-server:v0.0.1 .

docker run -d --name qwq-server \
  -p 5000:5000 -p 80:80 -p 443:443 \
  -v /qwq/qwq-server/resources:/app/resources \
  -v /qwq/qwq-server/configs:/app/configs \
  -v /qwq/qwq-server/web:/app/web \
  qwq-server:v0.0.1
