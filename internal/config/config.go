...
var (
	globalConfig = singleton.NewSingleton[Config]()
)
...
func Initialized(filenames ...string) (*Config, error) {
	var filename string
	if len(filenames) == 0 {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("配置初始化失败: %v", err))
	}
...
func New(newFilenames ...string) (*Config, error) {
	return globalConfig.Get(func() (*Config, error) {
		return Initialized(newFilenames...)
	})
}

func IsInitialized() bool {
	return globalConfig.IsInitialized()
}
